...
total.plot %>% group_by(Group, Authoritarianism) %>%
  subset(Year == 2004) %>% summarize(mean(Probability))


total.plot %>% group_by(Group, Authoritarianism) %>%
  subset(Year == 2008) %>% summarize(mean(Probability))

plot1<-ggplot(subset(total.plot, Group != "Independent"), aes(x=Probability,
                              y=as.factor(Year),
